...
const Index = () => {
  ...
  const handleDragStart = (e, jeton) => {
    const rect = dropRef.current.getBoundingClientRect();
    setDragOffset({ x: e.clientX - rect.left - jeton.x, y: e.clientY - rect.top - jeton.y });
  };

  const handleDrag = (e, jeton) => {
    if (e.clientX !== 0 && e.clientY !== 0) {
      const rect = dropRef.current.getBoundingClientRect();
      const newX = e.clientX - rect.left - dragOffset.x;
      const newY = e.clientY - rect.top - dragOffset.y;
      setJetons((prevJetons) => prevJetons.map((j) => (j.id === jeton.id ? { ...j, x: newX, y: newY } : j)));
    }
  };
  ...
  return (
    <Box textAlign="center">
      ...
      <Box position="relative" w={400} h={400} mx="auto" ref={dropRef}>
      ...
      <Button mt={8} colorScheme="green" onClick={() => setIsSpinning(true)} disabled={isSpinning || jetons.length === 0}>
        {isSpinning ? "Spinning..." : "Spin"}
      </Button>
      {jetons.map((jeton, index) => (
        <Box key={index} style={{ position: "absolute", left: jeton.x, top: jeton.y, cursor: "move" }} draggable>
          <Box as="svg" viewBox="0 0 100 100" width="50px" height="50px">
